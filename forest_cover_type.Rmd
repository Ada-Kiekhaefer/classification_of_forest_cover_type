...
* Accuracy, Precision, and Recall of all classes are shown in black, blue, and red lines, respectively. 
...
log_byclass %>%
  ...
  geom_line(aes(x=1:7, y=Precision), color='Blue') +
  geom_line(aes(x=1:7, y=Recall), color='Red') +
...
* The random forest model (cv = 2, mtry = 14) has overall accuracy of 94.14% with accuracy of each class ranging between 0.90 (Class 5) - 0.98 (Class 3). The precision of all classes range between 0.92 (Class 4) - 0.97 (Class 7) and recall range between 0.79 (Class 5) - 0.97 (Class 2). The recall of Class 5 is lowest at 0.79
...
ran_byclass %>%
  ...
  geom_line(aes(x=1:7, y=Precision), color='Blue') +
  geom_line(aes(x=1:7, y=Recall), color='Red') +
  scale_color_discrete(name = "Y series", breaks = c("Balanced.Accuracy", "Precision", "Recall")) +
  labs(x = 'Class', y = 'Value')
```